...
export class PythonConvertor {
    public static register(): void {
        ...
        Handlebars.registerHelper('parameterType', (typeObjec: string) => {
            const clonedParameterType = JSON.parse(JSON.stringify(typeObjec));
            return utils.solTypeToPyType(clonedParameterType);
        });
        Handlebars.registerHelper('returnType', (cReturnType: string) => {
            const clonedReturnType = JSON.parse(JSON.stringify(cReturnType));
            return utils.solTypeToPyType(clonedReturnType);
        });
        ...
        Handlebars.registerHelper('makeReturnDocstringRole', (description: string, indent: number) =>
            new Handlebars.SafeString(
                utils.wrapPythonDocstringRole(`:returns: ${description.replace(/  +/g, ' ')}`, indent),
            ),
        );
        Handlebars.registerHelper('makeEventParameterDocstringRole', (eventName: string, indent: number) =>
            new Handlebars.SafeString(
    ...
    public static solValueToPyValue(pythonVariable: string, abiItem: DataItem): string {
        const pythonTypeName = utils.solTypeToPyType(abiItem);
        // @ts-ignore
        if (pythonTypeName.match(/List\[.*\]/) !== null) {
            return `[${PythonConvertor.solValueToPyValue('element', {
                ...abiItem,
