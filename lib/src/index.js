...
for (const abiFileName of abiFileNames) {
    ...
    const methodsData = _.map(methodAbis, (methodAbi, methodAbiIndex) => {
        ...
        const methodData = Object.assign(Object.assign({}, methodAbi), { singleReturnValue: methodAbi.outputs.length === 1, hasReturnValue: methodAbi.outputs.length > 0, hasInputValue: methodAbi.inputs.length > 0, singleInputValue: methodAbi.inputs.length === 1, hasOutputs: _.has(methodAbi, 'outputs'), hasInputs: _.has(methodAbi, 'inputs'), isConstant: _.has(methodAbi, 'constant') && methodAbi.constant, languageSpecificName,
            functionSignature, isNonpayable: _.has(methodAbi, 'stateMutability') && methodAbi.stateMutability === 'nonpayable', isPayable: _.has(methodAbi, 'stateMutability') && methodAbi.stateMutability === 'payable', isView: _.has(methodAbi, 'stateMutability') && methodAbi.stateMutability === 'view', isPure: _.has(methodAbi, 'stateMutability') && methodAbi.stateMutability === 'pure', isPureOrView: _.has(methodAbi, 'stateMutability') && (methodAbi.stateMutability === 'pure' || methodAbi.stateMutability === 'view'), isPureOrViewOrConstant: _.has(methodAbi, 'stateMutability') && (methodAbi.stateMutability === 'pure' || methodAbi.stateMutability === 'view') || _.has(methodAbi, 'constant') && methodAbi.constant, devdoc: devdoc ? devdoc.methods[functionSignature] : undefined });
        return methodData;
    });
