...
exports.PythonConvertor = void 0;
...
class PythonConvertor {
    static register() {
        Handlebars.registerHelper('equal', (lhs, rhs) => {
            return lhs === rhs;
        });
        Handlebars.registerHelper('safeString', (str) => new Handlebars.SafeString(str));
        Handlebars.registerHelper('parameterType', (typeObjec) => {
            const clonedParameterType = JSON.parse(JSON.stringify(typeObjec));
            return utils_1.utils.solTypeToPyType(clonedParameterType);
        });
        Handlebars.registerHelper('returnType', (cReturnType) => {
            const clonedReturnType = JSON.parse(JSON.stringify(cReturnType));
            return utils_1.utils.solTypeToPyType(clonedReturnType);
        });
        Handlebars.registerHelper('toPythonIdentifier', utils_1.utils.toPythonIdentifier.bind(utils_1.utils));
        Handlebars.registerHelper('sanitizeDevdocDetails', (_methodName, devdocDetails, indent) => {
        ...
        Handlebars.registerHelper('makeParameterDocstringRole', (name, description, indent) => {
            let docstring = `:param ${name}:`;
            if (description && description.length > 0) {
                docstring = `${docstring} ${description}`;
            }
            return new Handlebars.SafeString(utils_1.utils.wrapPythonDocstringRole(docstring, indent));
        });
        Handlebars.registerHelper('makeReturnDocstringRole', (description, indent) => new Handlebars.SafeString(utils_1.utils.wrapPythonDocstringRole(`:returns: ${description.replace(/  +/g, ' ')}`, indent)));
        Handlebars.registerHelper('makeEventParameterDocstringRole', (eventName, indent) => new Handlebars.SafeString(utils_1.utils.wrapPythonDocstringRole(`:param tx_hash: hash of transaction emitting ${eventName} event`, indent)));
        Handlebars.registerHelper('tupleDefinitions', (abisJSON) => {
            ...
            for (const abi of abis) {
                ...
                if (abi.hasOwnProperty('inputs')) {
                    ...
                    parameters = parameters.concat(abi.inputs);
                }
                if (abi.hasOwnProperty('outputs')) {
                    ...
                    parameters = parameters.concat(abi.outputs);
                }
                for (const parameter of parameters) {
                    utils_1.utils.extractTuples(parameter, tupleBodies, tupleDependencies);
                }
            }
            ...
            // @ts-ignore
            tuplesToDeclare.push(...toposort_1.default(tupleDependencies));
        ...
        Handlebars.registerHelper('docBytesIfNecessary', (abisJSON) => {
            const abis = JSON.parse(abisJSON);
        ...
        Handlebars.registerHelper('toPythonClassname', (sourceName) => {
         ...
         */
        (pythonVariable, abiOutputs) => {
            if (abiOutputs.length === 1) {
                return new Handlebars.SafeString(PythonConvertor.solValueToPyValue(pythonVariable, abiOutputs[0]));
            }
            else {
                let tupleValue = '(';
    ...
    static solValueToPyValue(pythonVariable, abiItem) {
        const pythonTypeName = utils_1.utils.solTypeToPyType(abiItem);
        // @ts-ignore
        if (pythonTypeName.match(/List\[.*\]/) !== null) {
            return `[${PythonConvertor.solValueToPyValue('element', Object.assign(Object.assign({}, abiItem), { type: abiItem.type.replace('[]', '') }))} for element in ${pythonVariable}]`;
        }
        else {
...
exports.PythonConvertor = PythonConvertor;
//# sourceMappingURL=pythonssx.js.map