...
class utils {
    ...
    static solTypeToTsType(paramKind, backend, solType, components) {
        if (solType.match(exports.TRAILING_ARRAY_REGEX)) {
            ...
            if (backend === types_1.ContractsBackend.Tron) {
                solTypeRegexToTsType.push({ regex: '^u?int\\d*$', tsType: 'BigNumber' });
            }
            if (backend === types_1.ContractsBackend.Ethers) {
                solTypeRegexToTsType.push({ regex: '^u?int\\d*$', tsType: 'BigNumber' });
            }
            if (backend === types_1.ContractsBackend.Web3) {
                solTypeRegexToTsType.push({ regex: '^u?int\\d*$', tsType: 'BN' });
            }
            ...
            if (paramKind === types_1.ParamKind.Output) {
                if (backend === types_1.ContractsBackend.Ethers) {
    ...
    static isUnionTypeTs(tsType, sdk) {
        if (sdk === types_1.ContractsBackend.Tron) {
            return tsType === 'number|BigNumber';
        }
        else if (sdk === types_1.ContractsBackend.Web3) {
